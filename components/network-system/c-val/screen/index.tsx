...
import { cValIndexPath } from "@/components/network-system/c-val/screen/presenter";
import { useCValSocket } from "@/components/network-system/c-val/transport";
import type { CValScreenId } from "@/components/network-system/experiments";
...
const Canvas = styled.div<{ $scale: number }>`
  position: absolute;
  left: 0;
  top: 0;
...
export function CValInitialWrapper({
  screenId,
  snapshot,
}: {
  ...
  const direction =
    change > 0.005 ? "up" : change < -0.005 ? "down" : "steady";

  return (
    <Wrapper>
      <Market aria-label={`C-VAL ${screenId} initial financial market`}>
        <Reading>
          <p>MARKET INDEX</p>
...
export function CValScreenExperience({
  ...
  if (screenIds.length === 1) {
    return <Stage>{renderPane(screenIds[0], 1)}</Stage>;
  }

  return (
    <Stage>
      <WholeGrid>
        {screenIds.map((screenId) => renderPane(screenId, 0.5))}
      </WholeGrid>
    </Stage>
  );
}
