...
import type {
  DdongDitationArchiveEntry,
  DdongDitationSession,
} from "../model/types";
...
function LiveMark({ session }: { session: DdongDitationSession }) {
  return (
    <li
...
function ArchiveRow({ entry }: { entry: DdongDitationArchiveEntry }) {
  return (
...
export default function DdongDitationTwoScreen() {
  const { connected, snapshot } = useDdongDitationSocket("screen");
