...
import { createInitialCValSnapshot, type CValSnapshot } from "@/components/c-val/1/model";
import { useCValSocket } from "@/components/c-val/1/transport";
type CValOneLegacyScreenId = "casino-legacy" | "rollercoaster-legacy" | "news" | "media";
...
const Pane = styled.div<{ $screenId?: CValOneLegacyScreenId }>`
  position: relative;
  width: 100%;
  height: 100%;
  min-width: 0;
...
function ScreenContent({ screenId, snapshot }: { screenId: CValOneLegacyScreenId; snapshot: CValSnapshot }) {
...
export function CValScreenExperience({ screenIds }: { screenIds: readonly CValOneLegacyScreenId[] }) {
...
export default function CValScreen({ screenId }: { screenId: CValOneLegacyScreenId }) {
