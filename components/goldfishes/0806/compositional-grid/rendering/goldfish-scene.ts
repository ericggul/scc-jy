...
const MAX_COMPOSITION_CELL_COUNT = 72;
...
const ALIGNED_MEDIA_TOP_Y = FLOOR_SURFACE_Y + MAX_ATTENTION_PILLAR_EXTENT;
...
export class GoldfishScene {
  ...
  private readonly compositionMediaTile = new THREE.InstancedBufferAttribute(new Float32Array(MAX_COMPOSITION_CELL_COUNT), 1);
  private readonly compositionPillars: THREE.InstancedMesh;
  private readonly compositionMediaCells: THREE.InstancedMesh;
  ...
  private mediaTopAligned = true;
  private mediaSpeed = 24;
  private attentionCells: SelectedCell[] = [];
  private compositionCells: CompositionalCell[] = [];
  ...
  private readonly compositionPlaybackByCell = new Map<number, MediaPlayback>();
  private compositionPlayback: MediaPlayback[] = [];
  ...
  constructor({ canvas, fieldCanvas, count, color, paperColor, blockColor, cameraProjection = "perspective", fishModelStyle = "minimal" }: GoldfishSceneOptions) {
    ...
    const compositionPillarGeometry = new THREE.BoxGeometry(1, 1, 1);
    this.compositionMediaTile.setUsage(THREE.DynamicDrawUsage);
    compositionPillarGeometry.setAttribute("mediaTile", this.compositionMediaTile);
    this.compositionPillars = new THREE.InstancedMesh(
      compositionPillarGeometry,
      this.attentionPillarMaterial,
      MAX_COMPOSITION_CELL_COUNT,
    );
    this.compositionPillars.count = 0;
    this.compositionPillars.frustumCulled = false;
    this.compositionPillars.instanceMatrix.setUsage(THREE.DynamicDrawUsage);
    this.scene.add(this.compositionPillars);
    ...
    const compositionMediaGeometry = new THREE.PlaneGeometry(1, 1);
    compositionMediaGeometry.rotateX(-Math.PI / 2);
    compositionMediaGeometry.setAttribute("mediaTile", this.compositionMediaTile);
    this.compositionMediaCells = new THREE.InstancedMesh(
      compositionMediaGeometry,
      this.mediaMaterial,
      MAX_COMPOSITION_CELL_COUNT,
    );
    this.compositionMediaCells.count = 0;
    this.compositionMediaCells.frustumCulled = false;
    this.compositionMediaCells.instanceMatrix.setUsage(THREE.DynamicDrawUsage);
    this.compositionMediaCells.visible = false;
    this.scene.add(this.compositionMediaCells);
  ...
  private createMediaPlayback(
    cell: Pick<SelectedCell, "column" | "row">,
    ...
    void loadMediaAtlas(surface)
      .then((canvas) => {
        ...
        if (this.attentionSurface === surface) {
          ...
          this.compositionMediaCells.visible =
            this.compositionMediaCells.count > 0;
        }
      })
      .catch(() => {
  ...
  setAttentionSurface(surface: AttentionSurface) {
    ...
    this.setCompositionCells(this.compositionCells);
  }
  ...
  setMediaSpeed(speed: number) {
    ...
    for (const playback of this.compositionPlaybackByCell.values()) {
      playback.startedAt = this.lastElapsedMilliseconds;
      playback.duration = getPlaybackDuration(playback, this.mediaSpeed);
    }
  }

  setMediaTopAligned(aligned: boolean) {
    if (this.mediaTopAligned === aligned) return;
    this.mediaTopAligned = aligned;
    this.setAttentionCells(this.attentionCells);
    this.setCompositionCells(this.compositionCells);
  }
  ...
  setAttentionCells(cells: readonly SelectedCell[]) {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      const mediaTopY = this.mediaTopAligned
        ? ALIGNED_MEDIA_TOP_Y
        : FLOOR_SURFACE_Y + pillarExtent;
      const pillarCenterY = this.mediaTopAligned
        ? mediaTopY - pillarExtent
        : FLOOR_SURFACE_Y;
      ...
      this.local.position.set(cell.centerX - this.width / 2, mediaTopY + 0.02, cell.centerY - this.height / 2);
      this.local.rotation.set(0, 0, 0);
  ...
  setCompositionCells(cells: readonly CompositionalCell[]) {
    const count = Math.min(cells.length, MAX_COMPOSITION_CELL_COUNT);
    this.compositionCells = cells.slice(0, count);
    const mediaSurface =
      this.attentionSurface === "white" ? null : this.attentionSurface;
    const playback: MediaPlayback[] = [];
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      let cellPlayback: MediaPlayback | null = null;
      if (mediaSurface) {
        cellPlayback = this.compositionPlaybackByCell.get(cell.id) ?? null;
        if (!cellPlayback) {
          cellPlayback = this.createMediaPlayback(cell, mediaSurface);
          this.compositionPlaybackByCell.set(cell.id, cellPlayback);
        }
        playback.push(cellPlayback);
      }

      const pillarExtent = cell.pillarExtent * VERTICAL_EXTENT_MULTIPLIER;
      const mediaTopY = this.mediaTopAligned
        ? ALIGNED_MEDIA_TOP_Y
        : FLOOR_SURFACE_Y + pillarExtent;
      const pillarCenterY = this.mediaTopAligned
      ...
      this.local.scale.set(cell.width, pillarExtent * 2, cell.height);
      this.local.updateMatrix();
      this.compositionPillars.setMatrixAt(index, this.local.matrix);
      ...
      this.local.updateMatrix();
      this.compositionMediaCells.setMatrixAt(index, this.local.matrix);
      this.compositionMediaTile.setX(index, cellPlayback?.current ?? 0);
    }

    for (const id of this.compositionPlaybackByCell.keys()) {
      if (!activeIds.has(id)) this.compositionPlaybackByCell.delete(id);
    }

    this.compositionPlayback = playback;
    this.compositionPillars.count = count;
    this.compositionPillars.instanceMatrix.clearUpdateRanges();
    this.compositionPillars.instanceMatrix.addUpdateRange(0, count * 16);
    this.compositionPillars.instanceMatrix.needsUpdate = true;
    this.compositionMediaCells.count = count;
    this.compositionMediaCells.instanceMatrix.clearUpdateRanges();
    this.compositionMediaCells.instanceMatrix.addUpdateRange(0, count * 16);
    this.compositionMediaCells.instanceMatrix.needsUpdate = true;
    this.compositionMediaTile.clearUpdateRanges();
    this.compositionMediaTile.addUpdateRange(0, count);
    this.compositionMediaTile.needsUpdate = true;

    if (mediaSurface && count > 0) this.prepareMediaAtlas(mediaSurface);
    this.compositionMediaCells.visible =
      mediaSurface !== null && this.useMediaAtlas(mediaSurface) && count > 0;
  }
  ...
  private updateCompositionMediaPlayback(elapsedMilliseconds: number) {
    if (!this.compositionMediaCells.visible) return;
    let tileChanged = false;

    for (let index = 0; index < this.compositionPlayback.length; index += 1) {
      const playback = this.compositionPlayback[index];
      let playbackChanged = false;
      ...
      if (playbackChanged) {
        this.compositionMediaTile.setX(index, playback.current);
      }
    }

    if (tileChanged) {
      this.compositionMediaTile.clearUpdateRanges();
      this.compositionMediaTile.addUpdateRange(0, this.compositionPlayback.length);
      this.compositionMediaTile.needsUpdate = true;
    }
  }
  ...
  render(
    ...
    this.updateCompositionMediaPlayback(elapsedSeconds * 1000);
  ...
  getCompositionCellCount() {
    return this.compositionCells.length;
  }
  ...
  dispose() {
    ...
    this.compositionMediaCells.geometry.dispose();
    this.mediaMaterial.dispose();
    this.attentionPillars.geometry.dispose();
    this.compositionPillars.geometry.dispose();
