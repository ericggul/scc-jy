...
import {
  addCompositionalCell,
  createCompositionalGrid,
  createCursorField,
  createGrid,
  getAnchoredCells,
  getCellAtPoint,
  ...
  type CellAnchor,
  type CompositionalGridSettings,
  type CompositionalGridState,
  type CursorAgent,
  type Grid,
} from "../model";
import {
...
const DEFAULT_COMPOSITION_SETTINGS: CompositionalGridSettings = {
  density: 36,
  maximumSpan: 2,
  minimumLifetimeMilliseconds: 0,
  maximumLifetimeMilliseconds: 1000,
};
...
export default function Goldfishes3D({
  ...
  const compositionStateRef = useRef<CompositionalGridState | null>(null);
  const compositionNeedsResetRef = useRef(true);
  ...
  useControls(
    () => ({
      ...
      Arrangement: folder({
        "align image tops": {
          value: true,
          onChange: (aligned: boolean) => {
            mediaTopAlignedRef.current = aligned;
            sceneRef.current?.setMediaTopAligned(aligned);
          },
        },
      }),
      Composition: folder({
        "2×2+ blocks": {
          ...
          onChange: (enabled: boolean) => {
            ...
            if (!enabled) {
              ...
              sceneRef.current?.setCompositionCells([]);
            }
          },
        },
        "max cells": {
          ...
          onChange: (density: number) => {
            compositionSettingsRef.current = {
              ...compositionSettingsRef.current,
              density,
            };
            compositionNeedsResetRef.current = true;
  ...
  const selectTrace = useCallback(
    (points: readonly TracePoint[]) => {
      ...
      if (compositionEnabledRef.current) {
        ...
        sceneRef.current?.setCompositionCells(compositionState.cells);
      }
      redrawField();
    },
  ...
  useEffect(() => {
    ...
    scene.setMediaTopAligned(mediaTopAlignedRef.current);
    ...
    const sizeCanvases = () => {
      ...
      if (compositionEnabledRef.current) {
        compositionStateRef.current = createCompositionalGrid(
          grid,
          compositionSettingsRef.current,
          performance.now(),
        );
        ...
        scene.setCompositionCells(compositionStateRef.current.cells);
      } else {
        compositionStateRef.current = null;
        scene.setCompositionCells([]);
      }
      compositionNeedsResetRef.current = false;
      agentsRef.current = createCursorField(
    ...
    const render = (time: number) => {
      ...
      if (!reduceMotion.matches && grid) {
        ...
        if (compositionEnabledRef.current && compositionNeedsResetRef.current) {
          const nextCompositionState = createCompositionalGrid(
            grid,
            compositionSettingsRef.current,
            time,
          );
          for (const cell of getAnchoredCells(
            selectionRef.current,
            grid,
            width,
            height,
          ...
          compositionStateRef.current = nextCompositionState;
          compositionNeedsResetRef.current = false;
          scene.setCompositionCells(nextCompositionState.cells);
        } else if (!compositionEnabledRef.current && compositionStateRef.current) {
          compositionStateRef.current = null;
          scene.setCompositionCells([]);
        }
      }
      ...
      if (collectPerformanceMetrics) {
        ...
        if (metricDuration >= 2000) {
          ...
          interactionCanvas.dataset.performanceCompositionCells = String(
            scene.getCompositionCellCount(),
          );
