...
import type { LifeEventKind, MaterializedLifeEvent } from "./types";
...
export default function CalendarOneScreen() {
  ...
  const receiveSelection = useCallback(
    (selection: { profileIds: string[] }) => setProfileIds(selection.profileIds),
    [],
  );
                ...
                >
                  ...
                  <div className="grid gap-1 overflow-hidden">
                    {dayEvents.slice(0, 3).map((event) => (
                      <div
                        key={event.id}
                        className={`min-w-0 rounded border px-1.5 py-1 text-[10px] leading-tight sm:text-xs ${activeEventToneClasses[event.kind]}`}
                      >
