...
- `apps/c-val/scripts/generate-c-val-exclamations.mjs` runs the generation batches.
- `apps/c-val/scripts/collections/c-val-exclamations.json` holds the text, common acting
  direction, emotional presets, model, voice, and default batch size.
- `apps/c-val/scripts/collections/c-val-context-minimal-utterances.json` holds the
...
pnpm audio:c-val -- --run context-corpus-v02 \
  --corpus apps/c-val/scripts/collections/c-val-context-minimal-utterances.json \
  --dialect all --takes 1
```
