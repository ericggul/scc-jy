...
export const cValRoom = `experiment:${familyId}:${version}`;
...
async function saveSensorTrace(payload) {
  ...
  const directory = resolve(process.cwd(), "recordings", "c-val");
...
function register({ io, socket }) {
  ...
  socket.on(
    ...
    (payload = {}, acknowledge = () => {}) => {
      ...
      io.to(cValRoom).emit(events.recordingCommandOut, command);
...
export const cValExperiments = Object.freeze([cValExperiment]);
