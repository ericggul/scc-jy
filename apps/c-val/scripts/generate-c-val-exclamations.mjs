...
const defaultConfigPath = path.join(
  scriptRoot,
  "collections/c-val-exclamations.json",
);
...
const corpusPath = options.corpus
  ? path.resolve(process.cwd(), options.corpus)
  : null;
if (corpusPath) {
...
const outputRoot = path.resolve(appRoot, config.outputRoot);
...
process.stdout.write(
  `${options["dry-run"] ? "planning" : "generating"} ${tasks.length} take(s) in ${path.relative(repositoryRoot, outputDirectory)}\n`,
);
process.stdout.write(
...
if (await pathExists(outputDirectory)) {
  throw new Error(
    `Run already exists: ${path.relative(repositoryRoot, outputDirectory)}. Choose another --run name.`,
  );
}
...
const manifest = {
  ...
  run: runName,
  config: path.relative(repositoryRoot, configPath),
  corpus: corpusPath ? path.relative(repositoryRoot, corpusPath) : null,
  model: config.model,
  voice: config.voice,
...
try {
  await runWithConcurrency(tasks, concurrency, async (task) => {
    ...
    const record = {
      ...task,
      src: `/${path.relative(path.join(appRoot, "public"), targetPath)}`,
      ...(keepRaw
        ? {
            rawSrc: `/${path.relative(path.join(appRoot, "public"), rawPath)}`,
  ...
  await writeManifest();
  process.stdout.write(`complete: ${path.relative(repositoryRoot, outputDirectory)}\n`);
} catch (error) {
