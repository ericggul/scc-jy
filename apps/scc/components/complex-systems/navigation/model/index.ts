...
export type ComplexSystemExperiment = {
  id: string;
  href: string;
  group: string;
  groupLabel: string;
...
async function findArchiveRoots(): Promise<ArchiveRoots | null> {
  ...
  for (const candidate of candidates) {
    const components = path.join(candidate, "components", "complex-systems");
    if (!(await isDirectory(components))) continue;

    return {
      app: path.join(candidate, "app", "(complex-systems)"),
      components,
      docs: path.join(candidate, "docs", "experiments", "complex-systems"),
    };
  }
...
export async function getComplexSystemExperiments(): Promise<
  ComplexSystemExperiment[]
> {
    ...
    groups.map(async (group) => {
      ...
      return Promise.all(
        slugs.map(async (slug) => {
          ...
          const experiment: ComplexSystemExperiment = {
            id: `${group}/${slug}`,
            href: `/${group}/${slug}`,
            group,
            groupLabel,
  ...
  return experiments
    .flat()
    .filter((experiment): experiment is ComplexSystemExperiment => experiment !== null)
