...
export function nodeRadius(node: EconomyNode) {
  return sectorRadius[node.sector];
}

function radiusForId(id: string) {
...
function relationSide(relation: GraphRelation) {
  if (relation.facility) return 1;
  if (relation.inferred) return 1;
  if (relation.id.startsWith("impact:")) return relation.kind === "wage" || relation.kind === "procurement" ? -1 : 1;
  if (relation.kind === "loan") return -1;
  if (relation.kind === "wage" || relation.kind === "procurement") return -1;
  if (relation.kind === "consumption" || relation.kind === "tax" || relation.kind === "dividend") return 1;
  if (relation.kind === "bond") return relation.id.includes("redemption") ? 1 : -1;
  return relation.kind === "backstop" ? -1 : 1;
}

function routeGeometry(relation: GraphRelation, points: ReadonlyMap<string, GraphPoint>) {
  ...
  const start = {
    x: from.x + unit.x * (radiusForId(relation.from) + 5),
    y: from.y + unit.y * (radiusForId(relation.from) + 5),
  };
  const end = {
    x: to.x - unit.x * (radiusForId(relation.to) + 9),
    y: to.y - unit.y * (radiusForId(relation.to) + 9),
  };
...
/** Cubic lanes are assigned by relation semantics, so reverse claims and deposits visibly separate. */
export function relationPath(relation: GraphRelation, points: ReadonlyMap<string, GraphPoint>) {
  const route = routeGeometry(relation, points);
  if (!route) return "";
  return `M ${route.start.x} ${route.start.y} C ${route.first.x} ${route.first.y} ${route.second.x} ${route.second.y} ${route.end.x} ${route.end.y}`;
}

export function relationLabelPoint(relation: GraphRelation, points: ReadonlyMap<string, GraphPoint>) {
  const route = routeGeometry(relation, points);
