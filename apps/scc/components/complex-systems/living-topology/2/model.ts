...
const INITIAL_NODES = 220;
const LOCAL_RADIUS = 92;
...
function makeNode(id: number, x: number, y: number, age = 0): LocalNode {
  return {
    ...
    x,
    y,
    vx: (unit(id * 17.9) - 0.5) * 18,
    vy: (unit(id * 19.3) - 0.5) * 18,
    energy: 0.3 + unit(id * 3.1) * 0.62,
    age,
    metabolism: 0.035 + unit(id * 5.7) * 0.055,
    birthThreshold: 0.7 + unit(id * 7.3) * 0.22,
    deathThreshold: 0.06 + unit(id * 8.9) * 0.16,
    decisionPeriod: 0.12 + unit(id * 11.1) * 0.42,
    nextDecision: unit(id * 13.7) * 0.4,
  };
}
...
function makeEdge(id: number, source: number, target: number): LocalEdge {
  return {
    ...
    signal: unit(id * 4.3),
    age: 0,
    lifetime: 1.8 + unit(id * 6.1) * 7.5,
    decisionPeriod: 0.08 + unit(id * 9.7) * 0.35,
    nextDecision: unit(id * 12.7) * 0.35,
  };
}
...
export function createLocalNetwork(width: number, height: number): LocalNetwork {
  ...
  const nodes = Array.from({ length: INITIAL_NODES }, (_, index) => {
    ...
    return makeNode(id, x, y, unit(id * 17.3) * 9);
  });
...
export function stepLocalNetwork(
  ...
  for (const edge of network.edges) {
    ...
    const expires = age > edge.lifetime;
    const rejectsRelation =
      decidesNow && affinity < 0.34 && unit(edge.id * 101 + tick * 17) < 0.52;
  ...
  const updatedNodes = network.nodes.map((node) => {
    ...
    let accelerationX = (unit(node.id * 83 + tick * 0.71) - 0.5) * 13;
    let accelerationY = (unit(node.id * 89 + tick * 0.67) - 0.5) * 13;
    if (relations.length > 0) {
      const center = relations.reduce(
        (sum, related) => ({ x: sum.x + related.x, y: sum.y + related.y }),
        { x: 0, y: 0 },
      );
      accelerationX += (center.x / relations.length - node.x) * 0.14;
      accelerationY += (center.y / relations.length - node.y) * 0.14;
    }
    for (const other of neighborhood) {
      ...
      accelerationX += (dx / distanceSquared) * 520;
      accelerationY += (dy / distanceSquared) * 520;
    }
    ...
    if (
      (energy < node.deathThreshold && age > 0.7 && decisionNoise < 0.72) ||
      age > 7 + unit(node.id * 31) * 15
    ) {
      dead.add(node.id);
      return {
        ...node,
  ...
  for (const node of aliveNodes) {
    ...
    const candidates = localNodes(node, nextGrid)
      .filter((other) => {
        if (other.id === node.id) return false;
        if (currentKeys.has(edgeKey(node.id, other.id))) return false;
        return Math.hypot(other.x - node.x, other.y - node.y) < LOCAL_RADIUS;
      })
      .sort((a, b) => {
    ...
    if (!target || unit(node.id * 71 + tick * 37) < 0.47) continue;
