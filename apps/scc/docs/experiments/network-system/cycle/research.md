...
Full ledger: [source-ledger.csv](./source-ledger.csv).
...
- Add `apps/scc/socket/experiments/network-system/cycle/model.mjs`; it must not import the macro-economy model.
...
- Do not change any file under `components/network-system/macro-economy/` or
  `apps/scc/socket/experiments/network-system/macro-economy/*` during model replacement.
