...
4. **Interaction job:** the lower overlay switches calculation method, exposes damping, step rate, surfer count, and the preferential-attachment parameters. The untouched default is the source-like 100-page / 196-link graph in random-surfer mode with surfer watching enabled; a separate action constructs a 160-page / 471-link extension. Dragging changes only browser-side layout coordinates.
...
- **Changed variable:** the runnable browser field retains the source model's graph representation while exposing its parameters through `face-voronoi/4`'s minimal lower control grammar. Small static examples were removed; the original seeded 100-page / 196-link construction remains the default, alongside an opt-in 160-page / 471-link extension with a broader spring-layout calibration.
