...
import { unboundedRange } from "./unbounded-number";
...
export default function Goldfishes3D({
  ...
  const sideRotationRef = useRef(false);
  const sideRotationSpeedRef = useRef(0.16);
  ...
  const floorVisibleRef = useRef(false);
  ...
  useControls(
    () => ({
      ...
      Motion: folder({
        speed: unboundedRange(1, 0, 3, 0.1, (speed: number) => {
            motionSpeedRef.current = speed;
          }),
        depth: unboundedRange(64, 0, 140, 2, (value: number) => {
            renderSettingsRef.current.depth = value;
          }),
        "tail motion": unboundedRange(0.38, 0, 0.7, 0.01, (value: number) => {
      ...
      Camera: folder({
        ...
        "auto rotate": {
          value: false,
          onChange: (enabled: boolean) => {
            sideRotationRef.current = enabled;
          },
        },
        "rotation speed": unboundedRange(0.16, 0, 1, 0.01, (speed: number) => {
            sideRotationSpeedRef.current = speed;
          }),
        "reset view": button(() => {
      ...
      Field: folder({
        ...
        "image speed": unboundedRange(24, 0, 40, 1, (speed: number) => {
        ...
        "show floor": {
          ...
          onChange: (visible: boolean) => {
            floorVisibleRef.current = visible;
            sceneRef.current?.setFloorVisible(visible);
          },
        },
        "corner +": {
          value: false,
      ...
      Appearance: folder({
        ...
        "fin opacity": unboundedRange(0.76, 0.2, 1, 0.02, (value: number) => {
  ...
  useEffect(() => {
    ...
    scene.setFloorVisible(floorVisibleRef.current);
    ...
    const render = (time: number) => {
      ...
      if (!reduceMotion.matches && grid) {
        ...
        const motionDeltaSeconds =
          deltaSeconds * motionSpeedRef.current;
        motionElapsedSeconds += motionDeltaSeconds;
      ...
      if (!reduceMotion.matches && sideRotationRef.current) {
        scene.rotateSideView(deltaSeconds, sideRotationSpeedRef.current);
      }
