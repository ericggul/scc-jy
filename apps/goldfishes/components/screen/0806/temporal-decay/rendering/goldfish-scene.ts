...
export const TEMPORAL_DECAY_RENDERER_REVISION = 2;
...
function getTemporalCellKey(cell: Pick<TemporalCell, "id">) {
  return cell.id;
}
...
export class GoldfishScene {
  ...
  private readonly mediaStrataFrozenAt = new THREE.InstancedBufferAttribute(
    new Float32Array(MAX_MEDIA_STRATA_COUNT),
    1,
  );
  ...
  private readonly mediaStrataCellKeys = Array<string>(
  ...
  constructor({ canvas, fieldCanvas, count, color, paperColor, blockColor, cameraProjection = "perspective", fishModelStyle = "minimal" }: GoldfishSceneOptions) {
    ...
    this.mediaStrataFrozenAt.setUsage(THREE.DynamicDrawUsage);
    ...
    mediaStrataGeometry.setAttribute("segmentFrozenAt", this.mediaStrataFrozenAt);
    this.mediaStrataMaterial = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        attribute float segmentFrozenAt;
        ...
        void main() {
          ...
          float effectiveElapsedSeconds = segmentFrozenAt < 0.0
            ? elapsedSeconds
            : min(elapsedSeconds, segmentFrozenAt);
          float distanceBelowPresent = max(
            0.0,
            effectiveElapsedSeconds - segmentEndedAt
          ) * growthRate;
  ...
  setMediaSpeed(speed: number) {
    ...
    for (let index = 0; index < this.mediaPlayback.length; index += 1) {
      ...
      if (!cell || cell.endedAt !== undefined) continue;
      this.addMediaStratum(
        getTemporalCellKey(cell),
        cell,
        playback.current,
        playback.startedAt,
        this.lastElapsedMilliseconds,
  ...
  setMediaStrataEnabled(enabled: boolean) {
    ...
    if (enabled) {
      for (let index = 0; index < this.mediaPlayback.length; index += 1) {
        const playback = this.mediaPlayback[index];
        if (this.attentionCells[index]?.endedAt !== undefined) continue;
        playback.startedAt = this.lastElapsedMilliseconds;
      }
    }
  }
  ...
  private addMediaStratum(
    ...
    this.mediaStrataFrozenAt.setX(index, cell.endedAt ?? -1);
  ...
  private pruneInactiveMediaState() {
    const activeCellKeys = new Set(
      this.attentionCells.map((cell) => getTemporalCellKey(cell)),
    );
    ...
    for (let index = 0; index < this.mediaStrataCount; index += 1) {
      ...
      if (nextIndex !== index) {
        ...
        this.mediaStrataFrozenAt.setX(
          nextIndex,
          this.mediaStrataFrozenAt.getX(index),
        );
  ...
  private updateMediaStrataPositions() {
    ...
    const cellsByKey = new Map(
      this.attentionCells.map((cell) => [getTemporalCellKey(cell), cell]),
    );
    ...
    this.mediaStrata.instanceMatrix.needsUpdate = true;
    this.mediaStrataFrozenAt.clearUpdateRanges();
    this.mediaStrataFrozenAt.addUpdateRange(0, count);
    this.mediaStrataFrozenAt.needsUpdate = true;
  }
  ...
  setTemporalCells(cells: readonly TemporalCell[]) {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      if (mediaSurface) {
        const key = getTemporalCellKey(cell);
  ...
  private getCurrentPillarLength(
    ...
    const startedAt = this.mediaStrataEnabled && playback
      ...
      : cell.createdAt;
    const effectiveElapsedSeconds = Math.min(
      elapsedSeconds,
      cell.endedAt ?? elapsedSeconds,
    );
    return Math.min(
      MAX_RENDERED_TEMPORAL_EXTENT,
      Math.max(0, effectiveElapsedSeconds - startedAt) *
        this.temporalGrowthRate,
    );
  }
