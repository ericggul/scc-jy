...
const INITIAL_CAMERA_ELEVATION = 0;
const MEDIA_FADE_PORTION = 0.92;
...
function getMediaFadeDuration(playback: MediaPlayback) {
  if (!Number.isFinite(playback.duration)) return 0;
  return playback.duration * MEDIA_FADE_PORTION;
}
...
export class GoldfishScene {
  ...
  private readonly mediaNextTile = new THREE.InstancedBufferAttribute(new Float32Array(MAX_ATTENTION_CELL_COUNT), 1);
  private readonly mediaBlend = new THREE.InstancedBufferAttribute(new Float32Array(MAX_ATTENTION_CELL_COUNT), 1);
  ...
  constructor({ canvas, fieldCanvas, count, color, paperColor, blockColor, cameraProjection = "perspective", fishModelStyle = "minimal" }: GoldfishSceneOptions) {
    ...
    this.attentionPillarMaterial = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        attribute float mediaNextTile;
        attribute float mediaBlend;
        varying vec3 vPillarNormal;
        varying vec2 vPillarUv;
        varying float vPillarMediaTile;
        varying float vPillarMediaNextTile;
        varying float vPillarMediaBlend;

        void main() {
          vPillarNormal = normal;
          vPillarUv = uv;
          vPillarMediaTile = mediaTile;
          vPillarMediaNextTile = mediaNextTile;
          vPillarMediaBlend = mediaBlend;
          gl_Position =
            projectionMatrix *
            modelViewMatrix *
            instanceMatrix *
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          vec3 currentMediaColor = texture2D(
            mediaAtlas,
            pillarMediaAtlasUv(vPillarMediaTile)
          ).rgb;
          vec3 nextMediaColor = texture2D(
            mediaAtlas,
            pillarMediaAtlasUv(vPillarMediaNextTile)
          ).rgb;
          vec3 mediaColor = mix(
            currentMediaColor,
            nextMediaColor,
    ...
    attentionPillarGeometry.setAttribute("mediaNextTile", this.mediaNextTile);
    attentionPillarGeometry.setAttribute("mediaBlend", this.mediaBlend);
    ...
    this.mediaNextTile.setUsage(THREE.DynamicDrawUsage);
    this.mediaBlend.setUsage(THREE.DynamicDrawUsage);
    mediaGeometry.setAttribute("mediaTile", this.mediaTile);
    mediaGeometry.setAttribute("mediaNextTile", this.mediaNextTile);
    mediaGeometry.setAttribute("mediaBlend", this.mediaBlend);
    this.mediaMaterial = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        varying float vMediaNextTile;
        varying float vMediaBlend;

        void main() {
          vMediaUv = uv;
          vMediaTile = mediaTile;
          vMediaNextTile = mediaNextTile;
          vMediaBlend = mediaBlend;
          gl_Position =
            projectionMatrix *
            modelViewMatrix *
            instanceMatrix *
      ...
      fragmentShader: `
        ...
        void main() {
          vec4 currentMediaColor = texture2D(
            mediaAtlas,
            mediaAtlasUv(vMediaTile)
          );
          vec4 nextMediaColor = texture2D(
            mediaAtlas,
            mediaAtlasUv(vMediaNextTile)
          );
          gl_FragColor = mix(
            currentMediaColor,
            nextMediaColor,
  ...
  rotateSideView(deltaSeconds: number, speed: number) {
    this.cameraAzimuth = THREE.MathUtils.euclideanModulo(
      this.cameraAzimuth + deltaSeconds * speed,
      Math.PI * 2,
    );
  ...
  setMediaSpeed(speed: number) {
    this.mediaSpeed = Math.max(0, speed);
  ...
  setAttentionCells(cells: readonly SelectedCell[]) {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      this.mediaNextTile.setX(index, cellPlayback?.next ?? 0);
      this.mediaBlend.setX(index, 0);
    }
    ...
    this.mediaNextTile.clearUpdateRanges();
    this.mediaNextTile.addUpdateRange(0, count);
    this.mediaNextTile.needsUpdate = true;
    this.mediaBlend.clearUpdateRanges();
    this.mediaBlend.addUpdateRange(0, count);
    this.mediaBlend.needsUpdate = true;
  ...
  private updateMediaPlayback(elapsedMilliseconds: number) {
    ...
    for (let index = 0; index < this.mediaPlayback.length; index += 1) {
      ...
      if (playbackChanged) {
        ...
        this.mediaNextTile.setX(index, playback.next);
      }

      const fadeDuration = getMediaFadeDuration(playback);
      const fadeStartedAt = playback.startedAt + playback.duration - fadeDuration;
      const rawBlend = fadeDuration > 0
        ? THREE.MathUtils.clamp(
            (elapsedMilliseconds - fadeStartedAt) / fadeDuration,
            0,
            1,
          )
        : 0;
      const smoothBlend = rawBlend * rawBlend * (3 - 2 * rawBlend);
      this.mediaBlend.setX(index, smoothBlend);
      blendChanged ||= smoothBlend > 0 || this.mediaSpeed > 0;
    }
    ...
    if (tileChanged) {
      ...
      this.mediaTile.needsUpdate = true;
      this.mediaNextTile.clearUpdateRanges();
      this.mediaNextTile.addUpdateRange(0, this.mediaPlayback.length);
      this.mediaNextTile.needsUpdate = true;
    }

    if (blendChanged) {
      this.mediaBlend.clearUpdateRanges();
      this.mediaBlend.addUpdateRange(0, this.mediaPlayback.length);
      this.mediaBlend.needsUpdate = true;
    }
  }
  ...
  setFinOpacity(opacity: number) {
    this.finMaterial.opacity = opacity;
  }
  ...
  render(
    ...
    mediaElapsedSeconds = elapsedSeconds,
  ) {
    this.updateMediaPlayback(mediaElapsedSeconds * 1000);
