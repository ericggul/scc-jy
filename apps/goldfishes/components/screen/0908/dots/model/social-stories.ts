...
export function maintainSocialStoryActivity(
  ...
  if (system.states.some((state) => state.status === "new")) return system;
  ...
  const index = available[Math.floor(choice.value * available.length)]!;
  const next = newStoryState(now, choice.seed);

  return {
    ...system,
    time: now,
    states: system.states.map((state, stateIndex) => stateIndex === index ? next.state : state),
    randomSeed: next.seed,
  };
}
