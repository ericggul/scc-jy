...
import type { AgentGlyph, GoldfishScene } from "../rendering/goldfish-scene";
...
const SESSION_STORAGE_KEY = "goldfishes:0908:overlay-2d-3:stories:v1";
...
export function InstagramSocialStoryTray() {
  ...
  const [agentGlyph, setAgentGlyph] = useState<AgentGlyph>("goldfish");
  const agentGlyphRef = useRef<AgentGlyph>("goldfish");
  ...
  useEffect(() => {
    ...
    const fail = (reason: unknown) => {
      ...
      failed = true;
      window.clearTimeout(timer);
      console.error("Goldfishes 0908 overlay-2d-3 renderer:", reason);
    };
    const syncTargets = () => {
    ...
    const resize = () => {
      ...
      applyLayout();
      if (school) scene.render(school.fish, agentGlyphRef.current);
    };
    const draw = () => {
      ...
      if (!document.hidden && !motion.matches) {
        ...
        scene.render(school.fish, agentGlyphRef.current);
      } else if (layoutChanged || targetsChanged) {
        scene.render(school.fish, agentGlyphRef.current);
      }
      previous = started;
      ...
      <section aria-label="Story surface test" className={styles.controls}>
        <div className={styles.controlActions}>
          ...
          <div className={styles.actions}>
            ...
            {(["goldfish", "person"] as const).map((glyph) => (
              <button aria-pressed={agentGlyph === glyph} key={glyph} onClick={() => { agentGlyphRef.current = glyph; setAgentGlyph(glyph); }} type="button">
                {glyph === "goldfish" ? "금붕어" : "졸라맨"}
              </button>
            ))}
