...
const SESSION_STORAGE_KEY = "goldfishes:0908:overlay-2d:stories:v1";
...
export function InstagramSocialStoryTray() {
  ...
  useEffect(() => {
    ...
    const fail = (reason: unknown) => {
      ...
      failed = true;
      window.clearTimeout(timer);
      console.error("Goldfishes 0908 overlay-2d renderer:", reason);
    };
    const syncTargets = () => {
    ...
    const resize = () => {
      ...
      applyLayout();
      if (school) scene.render(school.fish);
    };
    const draw = () => {
