...
- **Interaction job:** A full-screen black intro contains only the 15 px Times New Roman text `lamortdelamodernité` and uses a pointer cursor across its entire surface. Clicking anywhere or pressing Enter or Space starts immediately. The pure French tricolour appears first; once both image pools have decoded eight local images, the complete image-and-blend layer fades from opacity 0 to 1 over three seconds. A top-right `Pause` / `Reprendre` control or the Space key freezes and resumes both canvases and the audio at their current positions. No image or file is clickable.
- **Wrapper justification:** The left sequence is anchored to the left edge at two-thirds viewport width and the right sequence is anchored to the right edge at two-thirds viewport width. Their shared central third is produced by `mix-blend-mode: difference`. Above the imagery, the outer left and right thirds receive non-interactive French-flag blue (`#0055A4`) and red (`#EF4135`) overlays using the same difference blend mode.
