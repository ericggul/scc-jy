...
2. [`Tinkering as the SCC working method`](../../foundations/tinkering.md);
3. [`Common visual design guidelines`](../../foundations/design-guidelines.md);
...
6. `components/goldfishes/experiments.ts`, which is the executable source of
...
`public/goldfishes/assets/company-logos`. Other existing public image files may
...
3. Create `components/goldfishes/MMDD/short-name` as a complete local copy. Use a
...
4. Add a matching short document under
   `docs/experiments/goldfishes/MMDD/short-name.md` before the trial's rationale
   is forgotten.
...
7. Register the route once in `components/goldfishes/experiments.ts` with its ISO
...
- Route names describe the operative material or relation, not chronology:
  `/goldfishes/0804/pillars`, not `/goldfishes/0804/2`.
