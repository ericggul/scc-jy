...
1. **낙하가 직관적이어야 한다.** 화면 위에서 출발해 아래의 축적층으로
   향하는 시작·중간·도착이 있어야 한다. 연기, 배경 노이즈, 추상 파도만으로
   낙하를 대체하지 않는다.
...
| `coiling-stream` | 급함이 풀려서 몸의 리듬이 느슨해지는 장면 | 굵은 한 줄이 고리치며 이어짐, 앞은 압력 있고 뒤는 풀림 |
| `split-stream` | 둘 이상의 시선·말·얼룩이 서로 간섭하는 장면 | 가운데와 양옆으로 갈라진 짧은 흐름, 불규칙한 재합류 |
...
| `before-after-poop` | 들어갈 때와 나올 때의 마음 | 떨어진 가방 · 닫힌 문 · 슬리퍼 | 급함 뒤에 고리치며 풀리는 굵은 한 줄 |
| `muddy-dog-husk` | 서로의 얼룩 보기 | 거울 · 두 슬리퍼 · 수건 | 서로 어긋나게 갈라져 떨어지는 세 줄 |
...
5. 새 운동 문법이 필요하면 `AccumulationMaterialKind`와 shader의 background,
   ...
   바꾸지 않는다.
6. `socket/experiments/ddong-meong/4/index.mjs`의 `contents`에 똑같은 slug와
   제목을 등록한다.
