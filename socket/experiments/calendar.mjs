...
function defaultSelection(variantId) {
  return {
    ...
    viewport: { row: 0, column: 0, rows: 10, columns: 3 },
    revision: 0,
    sentAt: Date.now(),
  };
}
...
function normalizeSelection(socket, payload = {}) {
  ...
  const rows = integerOr(viewport.rows, 10, 1, gridSize);
  const columns = integerOr(viewport.columns, 3, 1, gridSize);
  ...
  if (profileIds.length !== expectedCount) return null;
  ...
  return {
    ...
    profileIds,
    viewport: {
      row: clampInteger(viewport.row, 0, gridSize - rows),
      column: clampInteger(viewport.column, 0, gridSize - columns),
      rows,
      columns,
    },
